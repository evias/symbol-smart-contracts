...
export default class extends Contract {
  ...
  async execute(inputs: TransferSecurityInputs) 
  {
    ...
    if (sender.equals(target.address)) {
      console.log('')
      console.log(chalk.red('Target account cannot be used as a partition. Please, use the IssueSecurity contract for initial issuance.'))
      return ;
    }
    ...
    const senderPartition = token.getPartition(senderInfo.publicAccount)
    const recipientPartition = token.getPartition(recipient.publicAccount)
    ...
    const result = await token.transfer(
      ...
      recipientPartition.publicAccount, // partition (NEW RECIPIENT PARTITION)
      senderPartition.publicAccount, // sender (TRANSFER sends from partition account)
